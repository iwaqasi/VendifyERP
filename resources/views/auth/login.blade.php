...
@section('content')
    ...
    @if (config('app.env') == 'demo')
        <div class="tw-mb-4 tw-w-full" style="max-width: 560px;">
    ...
    <div class="vf-auth-card">
        <div class="vf-auth-card-head">
            <img class="logo-sm" src="{{ asset('img/vendify-logo-mark.png') }}" alt="{{ config('app.name') }} logo" />
            <h1>@lang('lang_v1.welcome_back')</h1>
            <p>@lang('lang_v1.login_to_your') {{ config('app.name') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" id="login-form">
            ...
            <div class="form-group has-feedback {{ $errors->has('username') ? ' has-error' : '' }}">
                ...
                <input
                    class="form-control vf-auth-input"
            ...
            <div class="form-group has-feedback {{ $errors->has('password') ? ' has-error' : '' }}">
                <label for="password">
                    ...
                    @if (config('app.env') != 'demo')
                        <a href="{{ route('password.request') }}"
                            class="tw-float-right tw-text-sm tw-font-medium" tabindex="-1">@lang('lang_v1.forgot_your_password')</a>
                    @endif
                </label>
                <div class="tw-relative">
                    ...
                    <button type="button" id="show_hide_icon" class="show_hide_icon"
                        style="position: absolute; top: 13px; right: 8px; z-index: 2;">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-eye tw-w-5" viewBox="0 0 24 24" stroke-width="1.5" stroke="#64748B" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="form-group tw-mt-1">
                <label class="tw-flex tw-items-center tw-gap-2 tw-font-medium tw-cursor-pointer" style="font-weight: 500;">
                    <input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}
                        class="tw-dw-checkbox" style="height: 16px; width: 16px;">
            ...
            <button type="submit" class="vf-btn-block tw-mt-2">
                @lang('lang_v1.login')
            </button>
        </form>

        <div class="vf-auth-foot">
            @if (!($request->segment(1) == 'business' && $request->segment(2) == 'register'))
                @if (config('constants.allow_registration'))
                    <span>{{ __('business.not_yet_registered') }}&nbsp;</span>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            if (passwordInput.attr('type') === 'password') {
                ...
                $('#show_hide_icon').html('<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-eye-off tw-w-5" viewBox="0 0 24 24" stroke-width="1.5" stroke="#64748B" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10.585 10.587a2 2 0 0 0 2.829 2.828"/><path d="M16.681 16.673a8.717 8.717 0 0 1 -4.681 1.327c-3.6 0 -6.6 -2 -9 -6c1.272 -2.12 2.712 -3.678 4.32 -4.674m2.86 -1.146a9.055 9.055 0 0 1 1.82 -.18c3.6 0 6.6 2 9 6c-.666 1.11 -1.379 2.067 -2.138 2.87"/><path d="M3 3l18 18"/></svg>');
            }
            else if (passwordInput.attr('type') === 'text') {
                passwordInput.attr('type', 'password');
                $('#show_hide_icon').html('<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-eye tw-w-5" viewBox="0 0 24 24" stroke-width="1.5" stroke="#64748B" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10 12a2 2 0 1 0 4 0a2 2 0 0 0 -4 0"/><path d="M21 12c-2.4 4 -5.4 6 -9 6c-3.6 0 -6.6 -2 -9 -6c2.4 -4 5.4 -6 9 -6c3.6 0 6.6 2 9 6"/></svg>');
            }
        });
        })
    </script>
