                ...
                <div class="box-body">
                    <div class="row">
                        <div class="col-sm-12 text-right">
                            ...
                            <a href="{{ action([\App\Http\Controllers\StaffScheduleController::class, 'index']) }}" class="btn btn-info"><i class="fa fa-clock-o"></i> Staff Schedules</a>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-12">
...
@include('restaurant.booking.create_with_services')
</section>
<!-- /.content -->
...
<script type="text/javascript">
    // Pass service products and staff to JavaScript BEFORE booking-services.js loads
    window.bookingServiceProducts = @json($service_products->values());
    window.bookingServiceStaff = @json($correspondents_for_js);
</script>
<script src="{{ asset('js/staff-schedule-filter.js?v=' . $asset_v) }}"></script>
<script src="{{ asset('js/booking-services.js?v=' . $asset_v) }}"></script>
    ...
    $(document).ready(function(){
        clickCount = 0;
            ...
            todays_bookings_table = $('#todays_bookings_table').DataTable({
                            ...
                            dom:'frtip',
                            "ajax": {
                                "url": "/restaurant/bookings/get-todays-bookings",
                                "data": function ( d ) {
        ...
        $(document).on('click', '.convert-to-invoice', function(e){
            ...
            swal({
                ...
                text: 'This will create an invoice and open the POS screen for payment.',
                icon: 'info',
                buttons: true,
                dangerMode: false,
            }).then((willConvert) => {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.success == true) {
                                ...
                                // Open POS edit page for payment
                                if (result.pos_url) {
                        ...
                        error: function(xhr) {
                            console.error('Convert to invoice error:', xhr.responseText);
                            toastr.error('Error converting booking to invoice. Please try again.');
                        }
                    });
                }
            });
