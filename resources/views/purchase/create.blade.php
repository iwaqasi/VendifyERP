		...
		<div class="row">
			...
			<div class="col-sm-8">
				<div class="form-group">						<div class="input-group">
						...
						<button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#product_selector_purchase" style="margin-top: 5px;">
							<i class="fa fa-th-large"></i> Select Product
						</button>
				</div>
			</div>
			<div class="col-sm-2">
...
@section('javascript')
	...
	{{-- Product Selector Modal (loaded after jQuery) --}}
	@include('components.product_selector', ['id' => 'product_selector_purchase', 'on_select' => 'onProductSelected', 'categories' => $categories ?? []])
	...
	<script type="text/javascript">
		function onProductSelected(products) {
			...
			console.log('onProductSelected called with:', products);
			
			// Add each selected product to the purchase table sequentially
			// API returns 'id' (not 'product_id') and 'variation_id'
			var addNext = function(index) {
				if (index >= products.length) return;
				var p = products[index];
				var productId = p.product_id || p.id;
				var variationId = p.variation_id || null;
				console.log('Adding product:', productId, 'variation:', variationId);
				get_purchase_entry_row(productId, variationId);
				// Small delay between additions to let row_count increment properly
				setTimeout(function() { addNext(index + 1); }, 300);
			};
			addNext(0);
		}
	</script>
