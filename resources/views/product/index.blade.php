        ...
        $(document).ready(function() {
            product_table = $('#product_table').DataTable({
                ...
                stateSaveCallback: function(settings, data) {
                    localStorage.setItem('vendify_products_layout_v2_{{ session('business.id') }}', JSON.stringify(data));
                },
                stateLoadCallback: function(settings) {
                    var saved = localStorage.getItem('vendify_products_layout_v2_{{ session('business.id') }}');
                    if (!saved) return null;
                    try {
                        var state = JSON.parse(saved);
                        // Ignore stale states that don't match the current table structure
                        if (!state.columns || !settings.aoColumns || state.columns.length !== settings.aoColumns.length) return null;
                        return state;
                    } catch (e) {
                        return null;
                    }
                ...
                columns: [{
                    ...
                    @for($i = 1; $i <= 20; $i++)
                        {
                            data: 'product_custom_field{{ $i }}',
                            name: 'products.product_custom_field{{ $i }}',
                            visible: $('#cf_{{ $i }}').text().length > 0
                        },
                    @endfor
                    {
                        data: 'action',
