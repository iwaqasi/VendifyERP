@php 
    ...
    $has_purchase_perm = auth()->user()->can('view_purchase_price');
    $has_selling_perm = auth()->user()->can('access_default_selling_price');
    // checkbox, image, action, product, location, stock, type, category, brand, tax, sku = 11
    $colspan = 11 + ($has_purchase_perm ? 1 : 0) + ($has_selling_perm ? 1 : 0) + $cf_count;
@endphp
<table class="table table-bordered table-striped ajax_view hide-footer" id="product_table">
    <thead>
        <tr>
            ...
            <th class="not-export" data-pdf-include="image" style="width:80px;min-width:80px;max-width:80px">{{__('lang_v1.product_image')}} </th>
            ...
            @endfor
            <th class="not-export action-col">@lang('messages.action')</th>
        </tr>
    </thead>
    <tfoot>
        <tr>
