...
<aside class="side-bar tw-relative tw-hidden tw-h-full tw-bg-white tw-w-64 xl:tw-w-64 lg:tw-flex lg:tw-flex-col tw-shrink-0">
    ...
    <a href="{{route('home')}}" title="{{ Session::get('business.name') }}"
        class="vf-sidebar-brand tw-w-full tw-shrink-0">
        <img src="{{ asset('img/vendify-logo-mark.png?v=' . $asset_v) }}" alt="{{ config('app.name') }} logo" />
        <span class="vf-sidebar-brand-text">
            <span class="vf-sidebar-brand-name">{{ config('app.name') }}</span>
            <span class="vf-sidebar-brand-tagline">{{ Session::get('business.name') }}</span>
        </span>
        <span class="vf-sidebar-status" title="@lang('lang_v1.online')"></span>
    </a>

    <!-- Sidebar Search -->
    <div id="side-bar-search-wrap" class="tw-px-3 tw-pt-2 tw-pb-1 tw-border-r tw-border-gray-200 tw-shrink-0">
