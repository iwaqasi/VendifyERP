...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            $enabled_modules_raw = !empty(session('business.enabled_modules')) ? session('business.enabled_modules') : [];
            // Ensure enabled_modules is always an array (may be stored as JSON string)
            $enabled_modules = is_string($enabled_modules_raw) ? json_decode($enabled_modules_raw, true) ?? [] : $enabled_modules_raw;
            // Storage format: {"0":"purchases","1":"add_sale",...,"bookings":false}
            // Extract enabled module names from numeric keys, skip disabled (false) entries
            $enabled_modules = array_values(array_filter($enabled_modules, fn($v, $k) => is_int($k) && is_string($v), ARRAY_FILTER_USE_BOTH));
                        ...
                        if (auth()->user()->can('sell.create')) {
                            if (in_array('pos_sale', $enabled_modules)) {
                                ...
                                $sub->url(
                                    ...
                                    ['icon' => '', 'active' => request()->is('pos/dashboard')]
                                );
            ...
            //Adjustment dropdown (formerly stock adjustment)
            ...
            // ============================================================
            $menu->dropdown('Website / CMS', function ($sub) use ($is_admin) {
                if ($is_admin) {
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'dashboard']),
                        '📷 CMS Dashboard',
                        ['icon' => '', 'active' => request()->is('cms') || request()->is('cms/dashboard')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'designer']),
                        '🎨 Website Designer',
                        ['icon' => '', 'active' => request()->is('cms/designer')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'preview']),
                        '🌐 Live Preview',
                        ['icon' => '', 'active' => request()->is('cms/preview')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'pages']),
                        '📄 Pages',
                        ['icon' => '', 'active' => request()->is('cms/pages*')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'posts']),
                        '📝 Blog Posts',
                        ['icon' => '', 'active' => request()->is('cms/posts*')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'banners']),
                        '🖼️ Banners / Sliders',
                        ['icon' => '', 'active' => request()->is('cms/banners*')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'menus']),
                        '📋 Navigation Menu',
                        ['icon' => '', 'active' => request()->is('cms/menus*')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'testimonials']),
                        '⭐ Testimonials',
                        ['icon' => '', 'active' => request()->is('cms/testimonials*')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'faqs']),
                        '❓ FAQs',
                        ['icon' => '', 'active' => request()->is('cms/faqs*')]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\CmsController::class, 'media']),
                        '📁 Media Library',
                        ['icon' => '', 'active' => request()->is('cms/media*')]
                    );
                }
            })->order(20);
