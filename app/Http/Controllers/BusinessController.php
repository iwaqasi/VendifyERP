    ...
    */
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            if (! empty($notAllowed)) {
                return $notAllowed;
            }            // Fetch business early so business_type logic can access existing data
            $business_id = request()->session()->get('user.business_id');
            $business = Business::where('id', $business_id)->first();
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                ...
                'min_order_total_for_rp', 'max_rp_per_order', 'redeem_amount_per_unit_rp',
                'min_order_total_for_redeem', 'min_redeem_point', 'max_redeem_point', 'rp_expiry_period',
                'rp_expiry_type', 'custom_labels', 'weighing_scale_setting',
                'code_label_1', 'code_1', 'code_label_2', 'code_2', 'currency_precision', 'quantity_precision', 'business_type', ]); 

            // Auto-enable modules based on business type
            $businessType = $request->input('business_type');
            
            // Set business type config if type is present
            if ($businessType) {
                $business_details['business_type'] = $businessType;
                $business_details['business_type_config'] = json_encode([
                    'type' => $businessType,
                    'configured_at' => now()->toISOString(),
                ]);
            }
            
            // Map business types to their required module keys
            // 'booking' and 'kitchen' are standard enabled_modules keys
            // 'repair' and 'wholesale' are custom business-type flags
            $typeModules = [
                'saloon' => ['booking'],
                'repair' => ['repair'],
                'restaurant' => ['kitchen'],
            ...
            // Store type modules to merge AFTER form checkbox processing
            $businessTypeModules = !empty($businessType) && isset($typeModules[$businessType])
                ? $typeModules[$businessType]
                : [];
           ...
           // Get existing pos_settings
            ...
            //Enabled modules - merge form checkboxes with business type required modules
            $enabled_modules = $request->input('enabled_modules');
            if (is_null($enabled_modules)) {
                $enabled_modules = [];
            }
            // Ensure business type required modules are always included
            if (! empty($businessTypeModules)) {
                $enabled_modules = array_unique(array_merge($enabled_modules, $businessTypeModules));
            }
        ...
        try {
            ...
            if (! empty($settings_array['slides'])) {
                foreach ($settings_array['slides'] as $key => $value) {
                    $settings_array['slides'][$key]['image_url'] = ! empty($value['image']) ? url('uploads/img/'.$api_settings->business_id.'/'.$value['image']) : '';
                }
            }
        } catch (\Exception $e) {
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());
