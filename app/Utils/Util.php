...
class Util
{
    ...
    public function allModulesEnabled($business_id = null)
    {
        ...
        // Normalize: enabled_modules may be stored in different formats:
        // Old format: {"0":"purchases",...,"bookings":false} (numeric keys=enabled, named false=disabled)
        // New format: ["purchases", "add_sale", "bookings"] (simple array of enabled names)
        ...
        if (is_array($enabled_modules)) {
            ...
            foreach ($enabled_modules as $key => $value) {
                // Numeric key with string value = enabled module (old format)
                if (is_int($key) && is_string($value)) {
                    $result[] = $value;
                }
                // Named key with boolean true = enabled module (old format with explicit true)
                elseif (is_string($key) && $value === true) {
                    $result[] = $key;
                }
                // Named key with string value = enabled module
                elseif (is_string($key) && is_string($value)) {
                    $result[] = $key;
                }
            }
            $enabled_modules = array_values(array_unique($result));
        }
     ...
     * @return string
     */
    public function uploadFile($request, $file_name, $dir_name, $file_type = 'document', $businessId = null)
    {
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)->isValid()) {
            ...
            if ($file->getSize() <= config('constants.document_size_limit')) {
                ...
                // Per-business folder: uploads/{dir_name}/{business_id}/{filename}
                $targetDir = 'uploads/' . $dir_name;
                if ($businessId) {
                    $targetDir = 'uploads/' . $dir_name . '/' . $businessId;
                    // Create business subdirectory if it doesn't exist
                ...
                if ($file->storeAs($targetDir, $new_file_name, 'public')) {
    ...
    public function createUser($request)
    {
        $user_details = $request->only([
            ...
            'custom_field_2', 'custom_field_3', 'custom_field_4', 'guardian_name', 'id_proof_name', 'id_proof_number', 'permanent_address', 'current_address', 'bank_details', 'selected_contacts', 'is_enable_service_staff_pin', 'service_staff_pin', 'default_location_id',
        ]);

        // Handle default_location_id — empty string means no default
        if (empty($user_details['default_location_id'])) {
            $user_details['default_location_id'] = null;
        }
