...
foreach ($products as $productData) {
    ...
    // Download image to per-business folder
    if (!empty($productData['image'])) {
        $imgDir = public_path('uploads/img/' . $businessId);
        if (!is_dir($imgDir)) {
            mkdir($imgDir, 0755, true);
        }
        $imgFilename = strtolower(preg_replace('/[^a-zA-Z0-9_\-.]/', '_', pathinfo(parse_url($productData['image'], PHP_URL_PATH), PATHINFO_FILENAME))) . '.' . pathinfo(parse_url($productData['image'], PHP_URL_PATH), PATHINFO_EXTENSION);
        $imgPath = $imgDir . '/' . $imgFilename;
        
        if (!file_exists($imgPath)) {
            $ctx = stream_context_create(['http' => ['timeout' => 15, 'ignore_errors' => true]]);
            $imgData = @file_get_contents($productData['image'], false, $ctx);
            if ($imgData !== false) {
                file_put_contents($imgPath, $imgData);
                DB::table('products')->where('id', $productId)->update(['image' => $imgFilename]);
                echo "  Downloaded image for '{$productData['name']}'\n";
            } else {
                echo "  Failed to download image for '{$productData['name']}'\n";
            }
        } else {
            DB::table('products')->where('id', $productId)->update(['image' => $imgFilename]);
            echo "  Image already exists for '{$productData['name']}'\n";
        }
    }
